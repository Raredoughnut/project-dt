...
/* ========================================================================
   시드: 개발/데모용 샘플 테스트.
   - "what-donut" (sum): 도넛 유형 4종
   - "love-mbti"  (axis): 연애 MBTI 16종
...
async function seedTest(def: SeedTestDef) {
  // idempotent — 기존 테스트 삭제(cascade로 문항·선택지·결과·응시기록 함께 삭제)
  await db.delete(tests).where(eq(tests.slug, def.slug));
  ...
  const choiceCount = def.questions.reduce((n, q) => n + q.choices.length, 0);
  console.log(
    `  ✓ ${def.slug}: 문항 ${def.questions.length} · 선택지 ${choiceCount} · 결과카드 ${def.results.length}`
  );
}
...
/* ── "나의 연애 MBTI" (axis) — 공유 콘텐츠 재사용 ─────────────────────── */

const LOVE: SeedTestDef = {
  slug: LOVE_MBTI.slug,
  title: LOVE_MBTI.title,
  description: LOVE_MBTI.description,
  category: LOVE_MBTI.category,
  scoringType: "axis",
  questions: LOVE_MBTI.questions.map((q) => ({
    text: q.text,
    groupKey: q.choices[0]?.axis, // 축 라벨(energy/information/decisions/lifestyle)
    choices: q.choices.map((c) => ({
      label: c.label,
      scores: { [c.axis]: c.score },
    })),
  })),
  results: LOVE_MBTI.results,
};

async function seed() {
  console.log("시드 시작…");
  await seedTest(DONUT);
  await seedTest(LOVE);
  console.log("완료: 테스트 2개");
}
